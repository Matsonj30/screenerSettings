...
def finvizData():
    ...
    screenerPage = requests.get('https://finviz.com/screener.ashx?v=111&f=sh_float_u100,sh_relvol_o10&ft=4&o=volume', headers = headers).text
    ...
    for name in names:
        ...
        """  if(int(int(change[index].strip('%'))) > 0): #green with volume
            sheetWrite.cell(row = 7, column = tempStartPoint).font = 'FF00FF00'
 """
